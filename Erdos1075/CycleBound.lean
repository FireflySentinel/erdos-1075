...
theorem cycle_upper_bound {n : ℕ} [NeZero n] (x : CycleVertex n → ℝ)
    ...
    (cycleGraph (finRotate n)).polynomial x ≤ baseDensity + 1 / ((4 : ℕ).factorial * (n : ℝ)) := by
  ...
  have hdel := (cycleGraph (finRotate n)).polynomial_zeroAt_bound (r := 4) (A k) x hx hmass.le
  have hlast : x (A k) / (4 : ℕ).factorial ≤ 1 / ((4 : ℕ).factorial * (n : ℝ)) := by
    calc
      x (A k) / (4 : ℕ).factorial ≤ (1 / n) / (4 : ℕ).factorial := div_le_div_of_nonneg_right hk (by positivity)
      _ = _ := by ring
