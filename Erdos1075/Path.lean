...
noncomputable def baseDensity : ℝ := 1 / 5 ^ 5
...
theorem path_bound (L : ℕ) (a b : ℕ → ℝ) (c u v w : ℝ) (d : Fin 3 → ℝ)
  ...
  have hPbound : u * cubicForm L a b c D + v * shiftedCubic L a b c D ≤
      q * t ^ 3 * (1 / 27 - (u / q) * (1 - u / q) * (D / t) ^ 3 / 10) := by
    calc
