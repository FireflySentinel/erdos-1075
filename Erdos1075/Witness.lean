...
def classSize (n : ℕ) : CycleVertex n → ℕ
  | Sum.inl _ => 4
  | Sum.inr (Sum.inl _) => 4
  | Sum.inr (Sum.inr (Sum.inl _)) => 4 * (n - 1)
  | Sum.inr (Sum.inr (Sum.inr (Sum.inl _))) => 2 * n
  | Sum.inr (Sum.inr (Sum.inr (Sum.inr (Sum.inl _)))) => 2 * n
  | Sum.inr (Sum.inr (Sum.inr (Sum.inr (Sum.inr (Sum.inl _))))) => 4 * n
  | Sum.inr (Sum.inr (Sum.inr (Sum.inr (Sum.inr (Sum.inr _))))) => 1

lemma classSize_sum (n : ℕ) (hn : 1 ≤ n) : (∑ v, classSize n v) = 24 * n := by
  simp [CycleVertex, Fintype.sum_sum_type, classSize]
  omega

lemma classSize_polynomial (n : ℕ) (hn : 1 ≤ n) (σ : Equiv.Perm (Fin n)) :
    (cycleGraph σ).polynomial (fun v => (classSize n v : ℝ)) = (4 * (n : ℝ)) ^ 6 + 4 * (n : ℝ) ^ 2 := by
...
noncomputable def witnessExcess (n : ℕ) : ℝ := 1 / (4 * (4 * (n : ℝ)) ^ 4)
...
lemma classSize_polynomial_density (n : ℕ) (hn : 1 ≤ n) (σ : Equiv.Perm (Fin n)) :
    (cycleGraph σ).polynomial (fun v => (classSize n v : ℝ)) =
      (1 + witnessExcess n) * ((24 * (n : ℝ)) / 6) ^ 6 := by
