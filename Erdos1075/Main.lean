...
lemma exists_cycle_bound_below (β : ℝ) (hβ : baseDensity < β) :
...
/-- Erdős #1075 is false for every uniformity at least sixteen.

The same positive excess works at arbitrarily large orders, and the density
bound holds for every nonempty vertex subset, including singletons.
-/
