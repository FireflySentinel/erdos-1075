...
Theorem 1 and its extension to every `r ≥ 6` in the
[current manuscript](PROOF.tex).

## Exact statement

For every natural number `r ≥ 6` and real `γ > 1 / r^r`, there is `ε > 0`
...
| Complete degree-6 open-path upper bound, with homogeneous normalization | [`Path.lean`](Erdos1075/Path.lean), `path_bound` |
...
| Elementary symmetric bound and the `1 / 5!` deletion estimate | [`Derivative.lean`](Erdos1075/Derivative.lean), `polynomial_zeroAt_bound` |
| Cycle upper bound `6^(-6) + 1 / (5! n)` | [`CycleBound.lean`](Erdos1075/CycleBound.lean), `cycle_upper_bound` |
...
`A_i` weight to zero opens the cycle and gives the same `1 / (5! n)` error bound.
